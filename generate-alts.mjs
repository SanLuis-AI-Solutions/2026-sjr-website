...
async function generateAlts() {
  const files = (await fs.readdir(dir)).filter(f => f.startsWith("bracelet-repair-") && f.endsWith(".jpg"));
  const altsJsonPath = "C:/Users/ninef/SanLuis Solutions projects/sjr-new-website-aiproject/src/lib/image-alts.json";
  ...
  for (const file of files) {
    ...
    const prompt = `Write a highly descriptive, SEO-optimized alt text (max 15 words) for this jewelry repair/sales/bracelet image. Be factual, use relevant keywords (e.g., gold watch, diamond ring setting, pearl necklace, jewelry repair, bracelet repair). Return ONLY the alt text string with no quotes.`;
  ...
  // Sort keys
  const sortedAlts = Object.keys(alts).sort().reduce(
    (obj, key) => {
      obj[key] = alts[key];
      return obj;
    },
  ...
  await fs.writeFile(altsJsonPath, JSON.stringify(sortedAlts, null, 2));
